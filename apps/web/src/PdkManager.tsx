  ...
  return <section className="private-pdk-panel">
    ...
    <details className="pdk-importer"><summary>{es?'Importar un PDK privado':'Import a private PDK'} <span>＋</span></summary>
      <div className="pdk-form">
        ...
        <label className="pdk-files">{es?'Paquetes del PDK':'PDK packages'}<input type="file" multiple accept=".zip,.tar,.gz,.tgz,.bz2,.xz,.lib,.db,.lef,.tlef,.v,.sv,.gds,.gdsii,.oas,.oasis,.spice,.cir,.cdl,.spi,.json,.lyt,.lyp,.lydrc,.lylvs,.map,.tf,.tluplus,.tcl,.rules,.cal,.pvs,.r,.c" onChange={event=>setFiles(Array.from(event.target.files??[]))}/><small>{files.length?`${files.length} ${es?'archivo(s) seleccionado(s)':'file(s) selected'}`:(es?'ZIP, TAR.GZ, TGZ o vistas individuales':'ZIP, TAR.GZ, TGZ, or individual views')}</small></label>
    ...
    {pdks.length>0&&<div className="private-pdk-list">{pdks.map(pdk=><article key={pdk.id}>
      ...
      <section className="pdk-converter">
        ...
        {pdk.conversion.translation&&<small className="pdk-translation">{es?'Intérprete comercial → abierto (borrador)':'Commercial → open interpreter (draft)'}: {pdk.conversion.translation.layer_count} {es?'capas':'layers'} · {pdk.conversion.translation.lvs_connection_count} LVS · {pdk.conversion.translation.rc_corner_count} RC {es?'esquinas':'corners'} · {pdk.conversion.translation.encrypted_file_count} {es?'cifrados no convertidos':'encrypted, not converted'} · {es?'requiere validación':'validation required'}</small>}
