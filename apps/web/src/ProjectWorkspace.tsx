...
function artifactKind(name:string):ArtifactKind {
  ...
  if(['.gds','.def','.lef','.gds.gz','.def.gz'].some(extension=>lower.endsWith(extension))) return 'layout'
...
function ArtifactBrowser({artifacts,locale,onDownload}:{artifacts:Artifact[];locale:'es'|'en';onDownload:(artifact:Artifact)=>void}) {
  ...
  const bundle=artifacts.find(artifact=>artifact.name==='signoff-package.zip')

  return <section className="artifact-browser">
    <div className="artifact-heading"><div><span>{es?'RESULTADOS GENERADOS':'GENERATED OUTPUTS'}</span><b>{artifacts.length} {es?'archivos':'files'} · {formatBytes(totalBytes)}</b></div><div className="artifact-heading-actions"><small>{es?'Seleccione un archivo para descargarlo':'Select a file to download it'}</small>{bundle&&<button onClick={()=>onDownload(bundle)}>↓ {es?'Descargar todo (.zip)':'Download all (.zip)'}</button>}</div></div>
...
export default function ProjectWorkspace({project,locale,onChange,onClose}:{project:StoredProject;locale:'es'|'en';onChange:(project:StoredProject)=>void;onClose:()=>void}) {
  ...
  const [clockPeriod,setClockPeriod]=useState(Number(initialPhysical.clock_period_ns??25))
  ...
  const [timingEffort,setTimingEffort]=useState<'balanced'|'aggressive'>(initialPhysical.timing_effort==='aggressive'?'aggressive':'balanced')
  ...
  async function runPhysical() {
    ...
    const physical={pdk:project.pdk,clock_port:clockPort,clock_period_ns:clockPeriod,die_width_um:dieWidth,die_height_um:dieHeight,core_utilization_pct:utilization,timing_effort:timingEffort}
    const currentManifest=readManifest(project)
    ...
    <WizardStep id="physical" number="04" title={es?'Implementación física':'Physical implementation'} summary="RTL → GDSII · LibreLane / OpenROAD" open={openSteps.has('physical')} onToggle={()=>toggleStep('physical')} status={physicalResult?(physicalResult.summary?.signoff_status?.toUpperCase()??(physicalResult.success?'PASS':'FAIL')):undefined}>
      {rtlSources.length>0&&['sky130A','gf180mcuD'].includes(project.pdk)?<div className="physical-panel">
        ...
        <div className="physical-config">
          ...
          <label>{es?'Ancho (µm)':'Width (µm)'}<input type="number" min="30" max="5000" value={dieWidth} onChange={event=>setDieWidth(Number(event.target.value))}/></label>
          <label>{es?'Alto (µm)':'Height (µm)'}<input type="number" min="30" max="5000" value={dieHeight} onChange={event=>setDieHeight(Number(event.target.value))}/></label>
        ...
        <p className="physical-guidance">{es?'Para PicoRV32: pruebe 25 ns en modo balanceado. Use 30 ns para una primera validación conservadora; active agresivo solo si necesita cerrar una meta exigente.':'For PicoRV32, try 25 ns in balanced mode. Use 30 ns for a conservative first validation; enable aggressive only for a demanding timing target.'}</p>
