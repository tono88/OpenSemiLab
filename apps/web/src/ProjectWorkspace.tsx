...
interface RunResult { job_id:string; engine:string; success:boolean; exit_code:number; output:string; duration_ms:number; artifacts:Artifact[] }
...
export default function ProjectWorkspace({project,locale,onChange,onClose}:{project:StoredProject;locale:'es'|'en';onChange:(project:StoredProject)=>void;onClose:()=>void}) {
  ...
  return <section className="project-workspace">
    ...
    {result&&<div className="console"><div><span>{result.engine} · {result.duration_ms} ms</span><b className={result.success?'success':'failed'}>{result.success?(es?'CORRECTO':'PASSED'):(es?'FALLÓ':'FAILED')} · EXIT {result.exit_code}</b></div><pre>{result.output}</pre>{result.artifacts.length>0&&<div className="artifact-list">{result.artifacts.map(artifact=><button key={artifact.name} onClick={()=>downloadArtifact(artifact)}><span>{es?'Descargar':'Download'} {artifact.name}</span><small>{artifact.size_bytes?`${(artifact.size_bytes/1024).toFixed(1)} KB`:artifact.media_type}</small></button>)}</div>}</div>}
  </section>
}
