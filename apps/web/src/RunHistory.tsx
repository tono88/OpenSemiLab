...
export default function RunHistory({runs,locale,onClear}:{runs:RunSnapshot[];locale:'es'|'en';onClear:()=>void}) {
  ...
  return <section className="run-history analysis-view">
    <div className="analysis-title"><div><span>{es?'COMPARACIÓN':'COMPARISON'}</span><b>{es?'Historial de ejecuciones':'Run history'}</b></div><button onClick={onClear}>{es?'Limpiar':'Clear'}</button></div>
    <div className="run-selector">{runs.map(run=><button className={compared.some(item=>item.id===run.id)?'active':''} onClick={()=>toggle(run.id)} key={run.id}><i className={run.success?'ok':'bad'}/><b>{run.action}</b><span>{run.engine}</span><small>{new Date(run.createdAt).toLocaleTimeString(locale,{hour:'2-digit',minute:'2-digit'})} · {run.duration_ms} ms</small></button>)}</div>
    <div className="comparison-table"><table><thead><tr><th>{es?'Métrica':'Metric'}</th>{compared.map(run=><th key={run.id}>{run.action}<small>{run.id}</small></th>)}</tr></thead><tbody><tr><td>{es?'Estado':'Status'}</td>{compared.map(run=><td key={run.id}>{run.success?(es?'Correcto':'Passed'):(es?'Falló':'Failed')}</td>)}</tr><tr><td>WNS (ns)</td>{compared.map(run=><td key={run.id}>{value(run.summary?.wns_ns)}</td>)}</tr><tr><td>TNS (ns)</td>{compared.map(run=><td key={run.id}>{value(run.summary?.tns_ns)}</td>)}</tr><tr><td>DRC</td>{compared.map(run=><td key={run.id}>{run.summary?.drc_violations??'—'}</td>)}</tr><tr><td>{es?'Área':'Area'} (µm²)</td>{compared.map(run=><td key={run.id}>{value(run.summary?.die_area_um2,0)}</td>)}</tr><tr><td>{es?'Curvas SPICE':'SPICE traces'}</td>{compared.map(run=><td key={run.id}>{run.simulation?.plots.reduce((sum,plot)=>sum+plot.series.length,0)??'—'}</td>)}</tr></tbody></table></div>
  </section>
}
