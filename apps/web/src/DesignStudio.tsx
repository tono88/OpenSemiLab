...
export default function DesignStudio() {
  ...
  return <main className="design-main">
    <section className="intro design-intro"><div><p className="eyebrow">DESIGN STUDIO / OPEN EDA</p><h1>Build the system.<br/><span>Inspect every stage.</span></h1><p>Choose what you want to create. OpenSemiLab assembles a teachable, reproducible flow from the IIC-OSIC toolchain instead of making you start from a list of applications.</p></div><div className="status-card"><span>EXECUTION BACKEND</span><b>IIC-OSIC-TOOLS</b><p>Planner available · isolated runner integration pending</p></div></section>
    ...
    {plan && <section className="flow-result"><div className="section-heading"><span>03</span><div><h2>{name}: engineering flow</h2><p>{plan.notice}</p></div></div><div className="flow-line">{plan.stages.map((item,index)=><article className="flow-stage" key={item.id}><div className="stage-index">{String(index+1).padStart(2,'0')}</div><div><span>{item.status==='ready'?'SPECIFICATION':'IIC-OSIC ADAPTER'}</span><h3>{item.title}</h3><p>{item.purpose}</p><div className="tool-list">{item.tools.map(tool=><i key={tool}>{tool}</i>)}</div><small>DELIVERS · {item.output}</small></div></article>)}</div><div className="runner-note"><div><span>RUNNER</span><b>{plan.runner}</b></div><strong>PLANNED</strong></div>{level==='expert'&&<pre>{JSON.stringify({schema:'opensemilab.design/v1',project:{name,kind,pdk,level,language},stages:plan.stages.map(s=>s.id)},null,2)}</pre>}</section>}
  </main>
}
