...
export default function DesignStudio({ locale }: { locale: 'es' | 'en' }) {
  ...
  return <main className="design-main">
    ...
    {activeProject&&<ProjectWorkspace project={activeProject} locale={locale} onChange={updateProject} onClose={()=>setActiveProjectId(null)}/>}
    ...
    {plan && <section className="flow-result"><div className="section-heading"><span>03</span><div><h2>{name}: {es?'flujo de ingeniería':'engineering flow'}</h2><p>{kind==='microcontroller'||kind==='fpga_prototype'?(es?'El análisis RTL, la simulación y la síntesis están conectados en el workspace. Los adaptadores de diseño físico continúan en desarrollo.':'RTL lint, simulation and synthesis are connected in the workspace. Physical design adapters remain pending.'):plan.notice}</p></div></div><div className="flow-line">{plan.stages.map((item,index)=><article className="flow-stage" key={item.id}><div className="stage-index">{String(index+1).padStart(2,'0')}</div><div><span>{item.status==='ready'?(es?'LISTO':'READY'):'IIC-OSIC ADAPTER'}</span><h3>{item.title}</h3><p>{item.purpose}</p><div className="tool-list">{item.tools.map(tool=><i key={tool}>{tool}</i>)}</div><small>{es?'ENTREGA':'DELIVERS'} · {item.output}</small></div></article>)}</div><div className="runner-note"><div><span>RUNNER</span><b>{plan.runner}</b></div><strong className={kind==='microcontroller'||kind==='fpga_prototype'?'connected':''}>{kind==='microcontroller'||kind==='fpga_prototype'?(es?'RTL · CONECTADO':'RTL · CONNECTED'):(es?'ARCHIVOS · CREADOS':'FILES · CREATED')}</strong></div>{level==='expert'&&<pre>{JSON.stringify({schema:'opensemilab.design/v1',project:{name,kind,pdk,level,language},stages:plan.stages.map(s=>s.id)},null,2)}</pre>}</section>}
  </main>
}
