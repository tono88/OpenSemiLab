...
const PDK_GUIDANCE:Record<string,{status:'integrated'|'partial'|'scaffold'|'research';titleEs:string;titleEn:string;detailEs:string;detailEn:string}>= {
  sky130A:{status:'integrated',titleEs:'RTL→GDSII integrado · PDK abierto experimental',titleEn:'Integrated RTL→GDSII · experimental open PDK',detailEs:'Apto para docencia, bloques endurecidos y chips de prueba. Producción comercial requiere el PDK, decks y aprobación vigentes de la fundición.',detailEn:'Suitable for teaching, hardened blocks, and test chips. Commercial production requires current foundry PDK, decks, and approval.'},
  gf180mcuD:{status:'integrated',titleEs:'RTL→GDSII integrado · PDK abierto experimental',titleEn:'Integrated RTL→GDSII · experimental open PDK',detailEs:'Flujo digital ejecutable con LibreLane. La distribución abierta GF180MCU se considera vista experimental, no certificación de producción.',detailEn:'Executable digital flow with LibreLane. The open GF180MCU distribution is an experimental preview, not production certification.'},
  'ihp-sg13g2':{status:'partial',titleEs:'Analógico/RF parcial · vista experimental',titleEn:'Partial analog/RF · experimental preview',detailEs:'Xschem, SPICE y openEMS están conectados; el flujo completo automatizado de layout, extracción y sign-off IHP todavía no está integrado.',detailEn:'Xschem, SPICE, and openEMS are connected; the complete automated IHP layout, extraction, and sign-off flow is not integrated yet.'},
  'ihp-sg13cmos5l':{status:'scaffold',titleEs:'Estructura de proyecto · integración pendiente',titleEn:'Project scaffold · integration pending',detailEs:'Permite organizar fuentes y configuración, pero todavía no ejecuta un flujo de fabricación completo.',detailEn:'It organizes sources and configuration, but does not yet execute a complete manufacturing flow.'},
  gt2n:{status:'research',titleEs:'PDK 2 nm de investigación · no fabricable desde OpenSemiLab',titleEn:'Research 2 nm PDK · not manufacturable from OpenSemiLab',detailEs:'GT2N aporta Liberty, LEF/GDS/CDL, modelos GAAFET y reglas ICV para benchmarking. Es una versión inicial con esquinas en desarrollo; no hay adaptador LibreLane calificado ni autorización de fundición.',detailEn:'GT2N provides Liberty, LEF/GDS/CDL, GAAFET models, and ICV rules for benchmarking. It is an initial release with corners under development; there is no qualified LibreLane adapter or foundry authorization.'},
}
...
export default function DesignStudio({ locale }: { locale: 'es' | 'en' }) {
  ...
  const [privatePdks,setPrivatePdks]=useState<PrivatePdk[]>([])
  const [activeProjectId,setActiveProjectId]=useState<string|null>(null)
  const activeProject=projects.find(project=>project.id===activeProjectId)??null
  const selectedPrivatePdk=privatePdks.find(item=>`private:${item.id}`===pdk)
  const pdkGuidance=PDK_GUIDANCE[pdk]??(selectedPrivatePdk?{
    status:selectedPrivatePdk.readiness.physical?'partial':'scaffold' as const,
    titleEs:selectedPrivatePdk.readiness.physical?'PDK privado · adaptador físico validado':'PDK privado · inventario local',
    titleEn:selectedPrivatePdk.readiness.physical?'Private PDK · validated physical adapter':'Private PDK · local inventory',
    detailEs:selectedPrivatePdk.readiness.physical?'LibreLane puede usar el adaptador privado montado en modo solo lectura. DRC/LVS/PEX conservan sus estados de validación independientes.':'El PDK está asociado al proyecto, pero RTL→GDSII seguirá deshabilitado hasta instalar un perfil LibreLane/OpenPDKs válido.',
  ...
  return <main className="design-main">
    ...
    <PdkManager locale={locale} onProfilesChange={setPrivatePdks} onSelect={profile=>{setPdk(`private:${profile.id}`);setPlan(null);document.querySelector('.design-config')?.scrollIntoView({behavior:'smooth'})}}/>
    ...
    <section className="design-config"><div className="section-heading"><span>02</span><div><h2>{es?'Configure el proyecto':'Configure the project'}</h2><p>{es?'La vista guiada explica las decisiones; el modo experto expone manifiestos y controles nativos.':'The guided view explains decisions; expert mode exposes manifests and native controls.'}</p></div></div>
      <div className="config-panel"><label>{es?'Nombre del proyecto':'Project name'}<input value={name} onChange={e=>setName(e.target.value)}/></label><label>{es?'Proceso / PDK':'Process / PDK'}<select value={pdk} onChange={e=>setPdk(e.target.value)}><option value="sky130A">SkyWater SKY130 · {es?'digital integrado':'integrated digital'}</option><option value="gf180mcuD">GlobalFoundries GF180MCU · {es?'digital integrado':'integrated digital'}</option><option value="ihp-sg13g2">IHP SG13G2 · {es?'RF/analógico parcial':'partial RF/analog'}</option><option value="ihp-sg13cmos5l">IHP SG13CMOS5L · {es?'pendiente':'pending'}</option><option value="gt2n">GT2N 2 nm GAAFET · {es?'solo investigación':'research only'}</option>{privatePdks.map(item=><option key={item.id} value={`private:${item.id}`}>{item.display_name} {item.version} · {es?'privado local':'local private'}</option>)}</select></label><label>{es?'Nivel de aprendizaje':'Learning depth'}<select value={level} onChange={e=>setLevel(e.target.value)}><option value="guided">{es?'Guiado':'Guided'}</option><option value="engineering">{es?'Ingeniería':'Engineering'}</option><option value="expert">{es?'Experto / parámetros nativos':'Expert / native parameters'}</option></select></label><label>{es?'Entrada principal':'Primary entry'}<select value={language} onChange={e=>setLanguage(e.target.value)}><option value="systemverilog">SystemVerilog</option><option value="verilog">Verilog</option><option value="vhdl">VHDL</option><option value="schematic">{es?'Esquemático':'Schematic'}</option></select></label><button className="run" onClick={buildPlan}>{es?'Crear flujo de diseño':'Create design flow'} <span>→</span></button></div>{pdkGuidance&&<div className={`pdk-guidance ${pdkGuidance.status}`}><b>{es?pdkGuidance.titleEs:pdkGuidance.titleEn}</b><span>{es?pdkGuidance.detailEs:pdkGuidance.detailEn}</span><small>{es?'El proceso queda fijado en el proyecto. Para migrar, cree una copia y vuelva a validar modelos, librerías, restricciones, layout y sign-off.':'The process is fixed in the project. To migrate, create a copy and revalidate models, libraries, constraints, layout, and sign-off.'}</small></div>}{error&&<p className="error">{error}</p>}
    </section>
